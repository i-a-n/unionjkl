...
Okay, but _how does it work?_ What precisely are the mechanicisms responsible for Redux’s behavior? Let’s look at the implementation:
...
So all that happens is the object above gets `dispatch`ed. `dispatch` calls `computeNextState`, which I believe to be the actual core of what Redux is, and is therefore worth looking at in detail.
